...
import Data.Tree.Game_tree.Game_tree as GT
import Data.Array
...
cchildren :: RNode -> [RNode]
cchildren (RNode field pC pT lM) = []
...
lookInDirectionFrom field pT (r,c) (dirRow, dirCol) = if ((field ! (r,c)) == ( 0 ,"*")) then countIt (r+dirRow) (c+dirCol) 0 else 0
    where checkIt nR nC = if ((nR <= ubR) && (nC <= ubC) && (nR >=lbR) && (nC >= lbC)) then True else False 
          countIt nR' nC' acc = if (checkIt nR' nC')    then 
